...
const deployCalyptusDefi: DeployFunction = async function (
    ...
    if (developmentChains.includes(network.name)) {
        ...
        const mockLendingPoolAAVE2 = await deployments.get(
            "MockLendingPoolAAVE2"
        );
        aave2LendingPool = mockLendingPoolAAVE2.address;

        const mockProtocolDataProviderAAVE2 = await deployments.get(
            "MockProtocolDataProviderAAVE2"
        );
    ...
    const priceFeedConsumer = await deploy("PriceFeedConsumer", {
        from: deployer,
        args: [daiUsdPriceFeed, linkUsdPriceFeed],
        log: true,
    ...
    const calyptusDefiAAVE2Factory = await deploy("CalyptusDefiAAVE2Factory", {
        from: deployer,
        args: [
            aave2LendingPool,
            aave2DataProvider,
    ...
    if (verifyContract && process.env.ETHERSCAN_API_KEY) {
        // Mumbai
        await verify(run, calyptusDefiAAVE2Factory.address, [
            aave2LendingPool,
            aave2DataProvider,
            priceFeedConsumer.address,
            feesCollector.address,
...
export default deployCalyptusDefi;
deployCalyptusDefi.tags = ["all", "CalyptusDefiAAVE2Factory"];
